...
namespace FirstNumShopifyApp.Controllers
{
    ...
    public class AccountController : Controller
    {
		...
		private async Task DeleteChargeId(ApplicationUser user)
		{
			...
			if (!update.Succeeded)
			{
				...
				string message = "Couldn't delete a user's Shopify charge id. Reason: " +
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
			...
			else
			{
				...
				string accessToken = user.ShopifyAccessToken;
				if (user.ShopifyChargeId.HasValue && string.IsNullOrEmpty(accessToken) == false)
				{
					var service = new ShopifyRecurringChargeService(domain, accessToken);
					...
					when ((int)e.HttpStatusCode == 404 /* Not found */)
					{
						...
						//Redirect the user to /register/charge
				...
				{
					//User has connected their Shopify shop, but hasn't accepted a subscription charge.
					return RedirectToAction("Charge", "Register");
				}
				else
				{
					//User has created an account, but hasn't connected their Shopify shop.
					return RedirectToAction("Connect", "Register");
				}
			}
		}
